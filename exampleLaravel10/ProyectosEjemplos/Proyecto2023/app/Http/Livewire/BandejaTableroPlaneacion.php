...
class BandejaTableroPlaneacion extends Component
{
    use WithPagination;
    ...
    protected $queryString = [
        'busqueda','mes', 'estatus', 'region', 'tipo_inspeccion', 'actividad_economica', 'asignado',
    ];
    ...
    public function mount()
    {
        ...
        $this->planeacion = Planeacion::with('auditorias')->where('anio', $this->anio)->first();
    ...
    {
        $this->filtrado = ($this->mes || $this->estatus || $this->region || $this->tipo_inspeccion || $this->actividad_economica || $this->asignado||$this->busqueda);
        $filtros = [
            'mes' => [
                'texto' => 'Mes:',
        ...
        return view('livewire.bandeja-tablero-planeacion', [
            'totales' => $auditorias->get(),
    ...
    public function getAuditorias(){
        ...
        $auditorias = PlaneacionAuditoriaEjecucion::with('grupo.planeacion')->whereHas('grupo.planeacion', function ($query) use ($current) {
            $query->where('anio', $current);
