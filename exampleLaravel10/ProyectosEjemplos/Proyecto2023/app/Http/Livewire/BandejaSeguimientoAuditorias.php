...
class BandejaSeguimientoAuditorias extends Component
{
    use WithPagination;
    ...
    public $tipo = null,
        ...
        $asignado = null,
        $cafta = null,
        $estatus_finalizado_id = null,
        $estatus_pendiente_id = null;
    ...
    public function mount()
    {
        ...
        $this->estatus_finalizado_id  = $catEstatus->elementos->where('codigo', 'finalizada')->first()->id; 

        $this->cat_estatus = $catEstatus->elementos->whereIn('id', [ $this->estatus_finalizado_id])->sortBy('nombre');
        $this->cat_estatus_array = $catEstatus->elementos->whereIn('id', [ $this->estatus_finalizado_id])->sortBy('nombre')->pluck('nombre', 'id')->toArray();
    ...
    public function render()
    {
        ...
        return view('livewire.bandeja-seguimiento-auditorias', [
    ...
    public function getAuditorias(){
        ...
        $auditorias = PlaneacionAuditoriaEjecucion::with('grupo')->when(true, function($query){
            $query->where('estatus_id',[$this->estatus_finalizado_id])->where('seguimiento', true);//Regla de Seguimiento Solo traer las finalizadas y seguimiento sin importar el año 
            //$query->where('estatus_id',[22]);//Esto es para pruebas busca los pendientes  
