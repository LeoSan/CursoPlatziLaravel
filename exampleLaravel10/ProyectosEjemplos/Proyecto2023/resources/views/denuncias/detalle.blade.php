...
@include('denuncias.partials.modal-confirm', [ 'denuncia'=>$denuncia,'id_boton'=>'btnModalSolicitudExpediente', 'msj_confirm'=>'Al solicitar el expediente se asume que no se realizará una providencia, por lo cual no podrá realizarla más adelante ¿Desea continuar?', 'nomBoton'=>'Continuar', 'accion'=>'modalSolicitarExpediente'])

<div class="container  detalle-denuncia px-0">
    <div class="row mb-3">
        <div class="col d-flex">
            <div class="title-principal">Denuncia {{$denuncia->folio}}</div>
    ...
    <div class="accordion rounded-0 border-0" id="accordionDetalle">
        <div class="accordion-item rounded-0 mb-2 border-0">
            ...
            <div id="collapseOne" class="accordion-collapse collapse @if ($denuncia->estatus->codigo == 'pendiente') {{'show'}} @endif" aria-labelledby="headingOne" data-bs-parent="#accordionDetalle">
        ...
        @if ($denuncia->estatus->codigo == 'en_revision' || $denuncia->hasEstatus('en_revision'))
        <div class="accordion-item rounded-0 mb-2 border-0">
            ...
            <div id="collapseTwo" class="accordion-collapse collapse  @if ($denuncia->estatus_id == obtenerIdCatalogoElementCodigo('en_revision') ) {{'show'}} @endif" aria-labelledby="headingTwo" data-bs-parent="#accordionDetalle">
                <div class="accordion-body bg-white">
                    {{-- Fragmento de información de Alta Denuncia --}}
                    @include('denuncias.fragmentos.info-alta-denuncia', ['denuncia' => $denuncia, 'doc_alta_denun'=>$doc_alta_denun])
                </div>
            </div>
        </div>
        @endif

        @if ($denuncia->estatus->codigo == 'solventado' || $denuncia->hasEstatus('solventado'))
        <div class="accordion-item rounded-0 mb-2 border-0">
            ...
            <div id="collapseThree" class="accordion-collapse collapse {{$denuncia->estatus->codigo=='solventado' ? 'show' : ''}}" aria-labelledby="headingThree" data-bs-parent="#accordionDetalle">
                <div class="accordion-body bg-white">
                    {{-- Fragmento de información de Providencia --}}
                    @include('denuncias.fragmentos.info-providencia', ['denuncia' => $denuncia, 'doc_providencia_denuncia'=>$doc_providencia_denuncia, 'col'=>'col-12'])
                </div>
            </div>
        </div>
        @endif

        @if ( $denuncia->estatus->codigo=='solicitud_expediente' || $denuncia->hasEstatus('solicitud_expediente') )
        <div class="accordion-item rounded-0 mb-2 border-0">
            ...
            <div id="collapseFour" class="accordion-collapse collapse @if ( $denuncia->estatus->codigo == 'solicitud_expediente' ||  $denuncia->estatus->codigo == 'expediente_recibido' || $denuncia->estatus->codigo == 'povidencia') {{'show'}}  @endif" aria-labelledby="headingFour" data-bs-parent="#accordionDetalle">
                <div class="accordion-body bg-white">
                    {{-- Fragmento de información de Solicitar Expediente --}}
                    @include('denuncias.fragmentos.info-solicitud-expediente', ['denuncia' => $denuncia, 'doc_solicitar_expe'=>$doc_solicitar_expe, 'col'=>'col-12'])
                </div>
            </div>
        </div>
        @endif
        ...
        @endif
        @if ($denuncia->estatus->codigo == 'inadmision' || $denuncia->estatus->codigo == 'revision_inadmision' || $denuncia->hasEstatus('inadmision') || $denuncia->hasEstatus('revision_inadmision') )
            <div class="accordion-item rounded-0 mb-2 border-0">
            ...
            <div id="collapseFive" class="accordion-collapse collapse  @if ( $denuncia->estatus->codigo == 'inadmision' || $denuncia->estatus->codigo == 'revision_inadmision'){{'show'}} @endif" aria-labelledby="headingFive" data-bs-parent="#accordionDetalle">
                <div class="accordion-body bg-white">
                    {{-- Fragmento de información de Inadmision --}}
                    @include('denuncias.fragmentos.info-inadmision', ['denuncia' => $denuncia, 'doc_oficio_inadmsion_denuncia'=>$doc_oficio_inadmsion_denuncia,'col'=>'col-12'])
                </div>
            </div>
        </div>
        @endif

        {{-- @if ( ($denuncia->estatus->codigo=='admitida' || $denuncia->hasEstatus('admitida')) && !$denuncia->informes?->count() ) --}}
        @if ( ($denuncia->estatus->codigo=='admitida' || $denuncia->hasEstatus('admitida'))  )
            <div class="accordion-item rounded-0 mb-2 border-0">
        ...
        @if ($denuncia->informe)
            <div class="accordion-item rounded-0 mb-2 border-0">
                ...
                <div id="collapseInformes" class="accordion-collapse collapse {{ ($denuncia->estatus->codigo == 'validar_informe_auditor' || $denuncia->estatus->codigo == 'informe_actualizado' || $denuncia->estatus->codigo == 'informe_cargado' || $denuncia->estatus->codigo == 'observaciones_informe' || $denuncia->estatus->codigo == 'finalizado') ? 'show' : '' }} " aria-labelledby="headingInformes" data-bs-parent="#accordionDetalle">
                    <div class="accordion-body bg-white">
                        {{-- Fragmento de información de Inadmision --}}
                        @include('denuncias.fragmentos.informe', ['informe' => $denuncia->informe, 'doc_informe_final'=>$doc_informe_final, 'doc_acuse_recibo_informe_final_denuncia'=>$doc_acuse_recibo_informe_final_denuncia])
                    </div>
                </div>
            </div>
        @endif
        ...
        {{-- Funcionalidad de botones --}}
            ...
            <div class="row mt-4 justify-content-end">
                <div class="form-group col-md-3 mb-2 mb-md-0 d-flex  justify-content-end px-2">
                    <a href="{{session('url_back_bandeja_denuncia')}}" class="btn btn-accion-detalle btn-default w-100">
                        Cancelar
                    </a>
                </div>
                @if($denuncia->usuario_asignado_id == auth()->id() ||  Auth::user()->can('gestionar_denuncias'))

                    @if (!in_array($denuncia->estatus->codigo, ['inadmision', 'finalizado', 'desestimado']))
                        @can('ejecutar_reasignar_auditor')
                            @include('denuncias.botones.acciones-st-reasignar-auditor', ['denuncia' => $denuncia])
                        @endcan
                    @endif
                    ...
                    @switch($denuncia->estatus->codigo)
                        @case('pendiente')
                            ...
                            @include('denuncias.botones.acciones-st-pendientes', ['denuncia' => $denuncia])
                        @break
                        @case('en_revision')

                            @include('denuncias.botones.acciones-st-revision', ['denuncia' => $denuncia])
                        @break
                        @case('solventado')

                            @include('denuncias.botones.acciones-st-solventado', ['denuncia' => $denuncia])
                        @break
                        @case('revision_inadmision')

                            @include('denuncias.botones.acciones-st-revision-inadmision', ['denuncia' => $denuncia])
                        @break
                        @case('expediente_recibido')

                            @can('admitir_denuncias')
                                <div class="form-group col-md-3 mb-2 mb-md-0 d-flex  justify-content-end">
                                    <a class="btn btn-accion-detalle bg-btn-guardar w-100" href="{{route('denuncias.showAdmision',$denuncia->id)}}">Admitir</a>
                                </div>
                            @endcan
                        @break
                        @case('admitida')

                            @can('cargar_informe_denuncia')
                                <div class="form-group col-md-3 mb-2 mb-md-0 d-flex  justify-content-end">
                                    <a class="btn btn-accion-detalle btn-success text-white w-100"
                                       href="{{route('denuncias.informe.crear',$denuncia->id)}}">
                                        Cargar informe
                                    </a>
                                </div>
                            @endcan
                        @break
                        @case('observaciones_informe')
                            @can('cargar_informe_denuncia')
                                <div class="form-group col-md-3 mb-2 mb-md-0 d-flex  justify-content-end">
                ...
                @switch($denuncia->estatus->codigo)
                    ...
                    @case('informe_cargado')
                    @case('informe_actualizado')
                        @can('comentar_informe_denuncia')
                            <div class="form-group col-md-3 mb-2 mb-md-0 d-flex  justify-content-end">
                                <a class="btn btn-accion-detalle bg-btn-reasignar-auditor text-white w-100"
                                   href="{{route('denuncias.informe.comentarios',$denuncia->id)}}">
                                    Comentar informe
                                </a>
                            </div>
                        @endcan
                        @can('ejecutar_finalizacion')
                            <div class="form-group col-md-3 mb-2 mb-md-0 d-flex  justify-content-end">
                                <a class="btn btn-accion-detalle bg-btn-guardar text-white w-100"
                                   href="{{route('denuncias.form.finaliza',$denuncia->id)}}">
                                    Finalizar
                                </a>
                            </div>
                        @endcan
